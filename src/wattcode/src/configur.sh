...
gen_watcom ()
{
  ...
  ../util/linux/mkmake -o watcom_s.mak -d build/watcom/small -v makefile.all WATCOM SMALL
  ../util/linux/mkmake -o watcom_l.mak -d build/watcom/large -v makefile.all WATCOM LARGE
  ../util/linux/mkmake -o watcom_f.mak -d build/watcom/flat -v makefile.all WATCOM FLAT
  ../util/linux/mkmake -o watcom_w.mak -d build/watcom/win32 -v makefile.all WATCOM WIN32

  ../util/linux/mkdep -s.obj -p\$\(OBJDIR\)/ *.[ch] > build/watcom/watt32.dep

  echo neterr.c: build/watcom/syserr.c >> build/watcom/watt32.dep
cd ../util
wmake -f errnos.mak wc_err
cd ../src
# these hacks won't work because errnos.c relies on being compiled as a target-exe.
